...
**Chạy full:**
```bash
python hf_sync.py pull --all                  # tải TOÀN BỘ data cũ từ HF (ner + thô + checkpoint)
python run_all.py                             # crawl + cleanup + NER (incremental)
python hf_sync.py push --all                  # đẩy toàn bộ data mới lên HF
```
...
> ⚠️ **LUÔN `pull --all` trước `push --all`**: push ghi đè file HF theo tên. Nếu chạy
> `run_all.py` mà KHÔNG pull `--all` trước, file thô rss/html local chỉ có data run đó →
> `push --all` sẽ ghi đè bản đầy đủ trên HF bằng bản nhỏ hơn → mất archive. Crawler có
> **circuit breaker** + **bucket 4h** — báo nào bị chặn IP (vd dantri) thì tự skip, không
> treo run. Chi tiết ở `CLAUDE.md` / `HF_MIGRATION.md`.
...
python hf_sync.py pull --all         # cần HF_TOKEN + HF_REPO_ID
python run_all.py
