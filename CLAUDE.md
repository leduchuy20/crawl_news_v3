...
Stage 4: Benchmark (optional)  (benchmark/)         → latency (ES vs CH vs PG) + storage (4 engines vs raw)
```
...
### Stage 4 — Benchmark (`benchmark/`, optional)

2 nhóm bench độc lập: **latency** (ES vs CH vs PG) và **storage** (4 engines + raw JSONL).

```bash
cd benchmark
make install          # psycopg2-binary, elasticsearch, clickhouse-driver, matplotlib, pymongo
make up               # news-pg + news-mongo container (cùng network main_process_news-net)
...
# ===== STORAGE (PG + Mongo + ES + CH vs raw JSONL) =====
python scripts/01_load_mongo.py --reset              # load 3 collections + indexes
python scripts/04_storage_compare.py                 # results/storage_<ts>.{json,csv,md}
python scripts/05_plot_storage.py --json results/storage_<ts>.json
# Hoặc gộp: make storage-all
```
...
`.github/workflows/crawl.yml` chạy cron `0 3 * * *` (10h VN, best-effort ±5-60 phút) +
manual `workflow_dispatch`. Steps: checkout → setup Python 3.11 → `pip install -r
requirements.txt` → `python run_all.py` → commit `data/*.jsonl data/*.json` ngược về
branch hiện tại. Timeout 330 phút (chừa cho NER lần đầu).

Yêu cầu repo settings: **Settings → Actions → General → Workflow permissions** =
**Read and write** (để bot push được). Upload `data/*.log` thành artifact 7 ngày.

## `.gitignore` đáng chú ý

```
data/*.log
data/articles_ready*.jsonl   ← KHÔNG commit (sản phẩm Stage 2)
__pycache__/, crawl_venv/, venv/, .venv/
...
| Vấn đề | Fix |
|---|---|
| `ModuleNotFoundError: backend` | Thiếu `PYTHONPATH=.` khi chạy uvicorn |
| `routing doesn't support values of type: VALUE_NULL` (ES 8.x) | Đã fix ở `02_index_to_es.py` |
| `PartiallyConsumedQueryError: Simultaneous queries` (CH) | `kb3/backend/deps/clients.py` đã dùng fresh client / request |
| `network main_process_news-net not found` (Stage 4) | Stage 2 chưa up |
| Mongo `Authentication failed` (Stage 4) | Dùng `--auth-db admin` (user `bench` được tạo ở admin DB qua MONGO_INITDB_ROOT_*) |
| `no input files matched` Stage 2 | Stage 1 chưa có NER output |
| ES OOM | Giảm `ES_JAVA_OPTS=-Xms512m -Xmx512m` trong `main_process/docker-compose.yml` |
| File > 90MB không rotate | Check đang dùng `JsonlStore` chứ không `open(..., 'a')` |

## TL;DR cho Claude

Khi user yêu cầu sửa code:
1. **Đọc PIPELINE.md trước** nếu user nói về flow chạy.
2. **Glob khi đọc dataset**, **`JsonlStore`/`JsonlWriter` khi ghi** — đừng phá rotation.
3. **Đồng bộ schema** giữa cleanup → ES mapping → CH schema khi thêm/đổi field.
4. **`01_cleanup` full rebuild, `02_ner` incremental** — đừng nhầm.
5. Stage 2 cần Docker, Stage 3 cần Stage 2, Stage 4 cần Stage 2 — check prerequisites.
6. RBAC: filter sửa ở `es_mapping.json` (alias) + `kb3/backend/deps/auth.py`.
